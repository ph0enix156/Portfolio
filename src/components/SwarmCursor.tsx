...
const SwarmCursor = ({
  ...
  useEffect(() => {
    ...
    const scatter = (cx: number, cy: number, intensity = 1) => {
      if (!propsRef.current.enabled) return;
      const escape = (620 + propsRef.current.speed * 130) * intensity;
      for (let i = 0; i < MAX; i++) {
        let dx = px[i] - cx;
        let dy = py[i] - cy;
      ...
      burst = Math.max(burst, Math.min(1.5, 0.85 * intensity));
    };
    ...
    const onScroll = () => {
      if (!propsRef.current.scatterOnScroll || !propsRef.current.enabled) return;
      const now = performance.now();
      const currentY = window.scrollY;
      const delta = Math.abs(currentY - lastScrollY);
      lastScrollY = currentY;

      if (delta > 4 && now - lastScrollTime > 75) {
        lastScrollTime = now;
        const cx = cursor.has ? cursor.x : cssW * 0.5;
        const cy = cursor.has ? cursor.y : cssH * 0.5;
        scatter(cx, cy, Math.min(1.4, 0.65 + delta * 0.012));
      }
    };
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      if (delta > 6 && now - lastScrollTime > 75) {
        lastScrollTime = now;
        const cx = cursor.has ? cursor.x : cssW * 0.5;
        const cy = cursor.has ? cursor.y : cssH * 0.5;
        scatter(cx, cy, Math.min(1.4, 0.6 + delta * 0.008));
      }
    };

    const isGlobal = propsRef.current.globalPointer;
    const targetEl: HTMLElement | Window = isGlobal ? window : container;

    targetEl.addEventListener('pointermove', onMove as EventListener, { passive: true });
    targetEl.addEventListener('pointerenter', onMove as EventListener, { passive: true });
    targetEl.addEventListener('pointerleave', onLeave as EventListener);
    ...
    return () => {
      ...
      targetEl.removeEventListener('pointerenter', onMove as EventListener);
      targetEl.removeEventListener('pointerleave', onLeave as EventListener);
